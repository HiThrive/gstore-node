 ...
 */
export const generateModel = (kind, schema, gstore) => {
    const model = class GstoreModel extends GstoreEntity {
            ...
            const onEntity = (entityDataFetched) => {
                ...
                if (ids.length === 1 &&
                    ...
                    if (this.gstore.config.errorOnEntityNotFound) {
                        throw new GstoreError(ERROR_CODES.ERR_ENTITY_NOT_FOUND, `${this.entityKind} { ${ids[0].toString()} } not found`);
                    }
                    return null;
                }
        ...
        static __compile(newKind, newSchema) {
            return generateModel(newKind, newSchema, gstore);
        }
        static __fetchEntityByKey(key, transaction, dataloader, options) {
            const handler = (keys) => {
                ...
                if (dataloader) {
                    ...
                    return dataloader.loadMany(keysArray).then((result) => arrify(result));
                }
        ...
        static __scopeHook(hook, args, hookName, hookType) {
            ...
            const getScopeForDeleteHooks = () => {
                const id = is.object(args[0]) && {}.hasOwnProperty.call(args[0], '__override') ? arrify(args[0].__override)[0] : args[0];
                if (is.array(id)) {
                    return null;
                }
                let ancestors;
